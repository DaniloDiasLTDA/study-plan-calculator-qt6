...
class UserTest(TestCase):
    def setUp(self):
        self.user = User.objects.create(
            name="Test Name", email="test@email.test", password="test2"
        )
    ...
    def test_email_field_rejects_invalid_format_on_full_clean(self):
        user = User(name="Test Name", email="Test Email", password="test_password")
    ...
    def test_unique_email_creation(self):
        user = User(name="testuser2", email="test@email.test", password="1223")
