...
from database import Announcement, User, db
...
try:
    user = User.create(name="Danilo Dias", email="dias@gmail.com", password="000111")
    user = User.create(name="Luis Souza", email=DEFAULT_USER_EMAIL, password="000111")
    user = User.create(
        name="Agatha Dantas", email="datas@outlook.com", password="000111"
    )
    user = User.create(
        name="ProgamadorPython", email="devpython@gmail.com", password="000111"
    )
...
announ_luiz = (
    Announcement.select().join(User).where(User.email == "example@example.com")
)
for announ in announ_luiz:
    logger.info(
        f"Anun id: {announ.id}, Title Anun: {announ.title},Value: {announ.value}"
    )
