...
with open(FILE_CSV, "w", newline="", encoding="utf-8") as file_csv:
    ...
    field_names = [
        ...
        "client_invoice",
        "paynotpay",
    ]  # Criando as chaves da lista
    writing = csv.DictWriter(
        file_csv, fieldnames=field_names
    )  # DictWriter cria um objeto e mapeia dicionarios em linhas
    ...
    for client in list_clients:
        if "client_name" in client and client["client_name"] == search_name:
            if "paynotpay" in client:
                ...
                print(
                    f"Aviso: Cliente '{search_name}' encontrado, mas a chave 'paynotpay' não existe."
                )
                return {
                    "name": client["client_name"],
                    "status": "Informação de status ausente",
                }
